...
In each competition there's always one winner and one loser; there
...
def tournamentWinner(competitions, results):
    ...
    for i in range(len(results)):
        if results[i] == 0:
            short_list.append(competitions[i][1])
        else:
            short_list.append(competitions[i][0])

    team = ''
    max_count = 0

    for item in set(short_list):
        count = short_list.count(item)
        if count > max_count:
            max_count = count
            team = item
